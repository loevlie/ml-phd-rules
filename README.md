...
The best researchers seem to operate by rules like these — rarely written down anywhere, just internalized through years of practice. This is an attempt to write them down, so you can hold yourself to them.
...
14. **Fix your random seed first.** Write a single `set_seed(seed)` helper that seeds Python, NumPy, PyTorch, and CUDA deterministically, and call it in every entry point. Always know which seed produced which result.
...
34. **Report it right:**
    ...
    - Keep one shared stats utility in your codebase so every table is computed the same way
...
37. **Shape-annotated signatures with `jaxtyping` + `beartype`:**
    ...
    def forward(
        ...
        ...
    ```
    Every function that touches tensors documents its shapes in the signature. `@jaxtyped(typechecker=beartype)` enforces both the per-argument types *and* the cross-argument axis-size consistency (e.g., the `batch` dim is checked to match across `x`, `labels`, and the return).
...
43. **Notebooks for exploration, scripts for production.** Notebooks are for EDA and visualization. Training, evaluation, and anything that runs on a cluster is a `.py` script with a typed config.
    - Notebook naming convention: `01-abc-initial-data-exploration.ipynb` (number, your initials, description)

---

## VIII. Experiment Workflow

44. **All configuration goes through typed configs.** No hardcoded hyperparameters in entry points. Every tunable value is a field in a typed config (e.g., pydantic dataclasses). Override from the command line:
    ```bash
...
47. **Multi-seed launcher.** Keep a script that launches the same experiment across N seeds and aggregates results:
    ```bash
...
55. **Every paper gets a project page.** Use the [Academic Project Page Template](https://github.com/eliahuhorwitz/Academic-project-page-template). Deploy to GitHub Pages. Include: abstract, key figure, results, BibTeX, links to paper/code/demo.
...
| What | Tool | Why |
|---|---|---|
| Config | pydantic + tyro | Typed configs, Hydra-style CLI overrides, auto-snapshots |
| Tracking | W&B (cloud, default) / trackio (local) / TensorBoard | Switch via config: local for sensitive data, cloud for public |
...
| Project page | [Academic Project Page Template](https://github.com/eliahuhorwitz/Academic-project-page-template) | Battle-tested, GitHub Pages |
